...
fn run() -> anyhow::Result<()> {
    ...
    fmt()
        ...
        .with_target(false)
        .with_writer(|| CrlfWriter { inner: std::io::stderr() })
        .init();
...
fn run_enter(
    ...
    if !root_dir.exists() {
        return Err(RootboxError::PathError(
...
fn run_overlay(
    ...
    let mode = if persist.is_some() { "persistent" } else { "ephemeral" };
    ...
    match unsafe { fork() } {
        Ok(ForkResult::Parent { child }) => {
            ...
            let wait_status = waitpid(child, None)
                .map_err(|e| RootboxError::ProcessError(
...
fn execute_command(command: &str, args: &[String]) -> Result<()> {
    ...
    nix::unistd::execvp(&c_args[0], &c_args)
        .map_err(|e| RootboxError::ExecError(
