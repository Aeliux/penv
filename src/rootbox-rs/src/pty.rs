...
use nix::sys::termios;
use nix::libc::{TIOCSCTTY, TIOCGWINSZ, winsize};
...
impl PtyManager {
    ...
    pub fn setup_pty(&mut self) -> Result<(RawFd, RawFd)> {
        ...
        let pty_result = openpty(&winsize, term_attrs.as_ref())
            .map_err(|e| RootboxError::PtyError(
    ...
    fn get_window_size(&self, fd: RawFd) -> winsize {
    ...
    pub fn setup_slave(&self, slave_fd: RawFd) -> Result<()> {
        ...
        dup2(slave_fd, libc::STDIN_FILENO)
            .map_err(|e| RootboxError::PtyError(
                format!("Failed to dup2 stdin: {}", e)
            ))?;
        
        dup2(slave_fd, libc::STDOUT_FILENO)
            .map_err(|e| RootboxError::PtyError(
                format!("Failed to dup2 stdout: {}", e)
            ))?;
        
        dup2(slave_fd, libc::STDERR_FILENO)
            .map_err(|e| RootboxError::PtyError(
                format!("Failed to dup2 stderr: {}", e)
            ))?;
        ...
        if slave_fd > libc::STDERR_FILENO {
            nix::unistd::close(slave_fd)
                .map_err(|e| RootboxError::PtyError(
                    format!("Failed to close slave fd: {}", e)
                ))?;
        }
        ...
        unsafe {
            if libc::ioctl(libc::STDIN_FILENO, TIOCSCTTY, 0) != 0 {
                warn!("Failed to set controlling terminal: {}", io::Error::last_os_error());
            }
        }
    ...
    pub fn set_raw_mode(&self) -> Result<()> {
        ...
        let mut termios = tcgetattr(unsafe { BorrowedFd::borrow_raw(stdin_fd) })
            .map_err(|e| RootboxError::PtyError(
...
termios::cfmakeraw(&mut termios);
        ...
        tcsetattr(unsafe { BorrowedFd::borrow_raw(stdin_fd) }, SetArg::TCSANOW, &termios)
            .map_err(|e| RootboxError::PtyError(
                format!("Failed to set raw mode: {}", e)
            ))?;
    ...
    pub fn io_loop_blocking(&self, master_fd: RawFd, _child_pid: Pid) -> Result<()> {
