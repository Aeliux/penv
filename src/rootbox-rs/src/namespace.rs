...
use nix::unistd::{Uid, Gid, getuid, getgid, getpid, sethostname};
...
impl NamespaceManager {
    ...
    pub fn setup_user_namespace(&self) -> Result<()> {
        ...
        unshare(CloneFlags::CLONE_NEWUSER)
            .map_err(|e| RootboxError::NamespaceError(
    ...
    fn setup_uid_map(&self) -> Result<()> {
        ...
        write_proc_file(&uid_map_path, &uid_map_content)
            .map_err(|e| RootboxError::NamespaceError(
                format!("Failed to write uid_map: {}", e)
            ))?;
    ...
    fn setup_gid_map(&self) -> Result<()> {
        ...
        write_proc_file(&setgroups_path, "deny\n")
            .map_err(|e| RootboxError::NamespaceError(
                format!("Failed to write setgroups: {}", e)
            ))?;
        ...
        write_proc_file(&gid_map_path, &gid_map_content)
            .map_err(|e| RootboxError::NamespaceError(
                format!("Failed to write gid_map: {}", e)
            ))?;
        ...
        if !flags.is_empty() {
            ...
            unshare(flags)
                .map_err(|e| {
                    ...
                    RootboxError::NamespaceError(
                        format!("Failed to unshare namespaces: {}", e)
                    )
                })?;
        }
    ...
    fn set_hostname(&self, hostname: &str) -> Result<()> {
        ...
        if let Some(domainname) = &self.config.namespaces.domainname {
            ...
            unsafe {
                ...
                if result != 0 {
                    warn!("Failed to set domainname: {}", std::io::Error::last_os_error());
                }
            }
        }
    ...
    pub fn setup_parent_death_signal(&self) -> Result<()> {
        ...
        unsafe {
            ...
            if result != 0 {
                warn!("Failed to set parent death signal: {}", std::io::Error::last_os_error());
            }
        }
    ...
    pub fn set_no_new_privs(&self) -> Result<()> {
        ...
        unsafe {
            ...
            if result != 0 {
                return Err(RootboxError::NamespaceError(
                    format!("Failed to set NO_NEW_PRIVS: {}", std::io::Error::last_os_error())
                ));
            }
        }
...
fn write_proc_file(path: &str, content: &str) -> std::io::Result<()> {
